...
var input_credit_button = document.getElementById('pp2_button')
input_credit_button.addEventListener('click', getResults)

// Gathers radio button values into array for DB
let scores = []
function getResults() {
  scores = []
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    // if any radio button is left unchecked
    let unchecked = true
  ...
  if (uncheckedRadioButtons.length > 0) {
    ...
    fetch('/pp_1_zavrseno', {
      ...
      .then((response) => {
        // Redirect to "zavrsetak.html" on success
        if (response.ok) {
